...
const matchBox = ()=>{
	...
	//Set color box
	if (scanPart.eyeOpen)
		$("#eyes-open").attr('class','gesture-box ok');

	if (scanPart.eyeClosed)
		$("#eyes-closed").attr('class','gesture-box ok');

	if (scanPart.smile)
		$("#smile").attr('class','gesture-box ok');

	if (scanPart.mouthOpen)
		$("#mouth-open").attr('class','gesture-box ok');

}

//Set the default mark.
const markRunBox = (num)=>{

	//Set color box
	if (num==1)
		$("#eyes-open").attr('class','gesture-box run');

	if (num==2)
		$("#eyes-closed").attr('class','gesture-box run');

	if (num==3)
		$("#smile").attr('class','gesture-box run');

	if (num==4)
		$("#mouth-open").attr('class','gesture-box run');
...
const markError = (num)=>{
	...
	//Set color box
	if (num==1)
		$("#eyes-open").attr('class','gesture-box error');

	if (num==2)
		$("#eyes-closed").attr('class','gesture-box error');

	if (num==3)
		$("#smile").attr('class','gesture-box error');

	if (num==4)
		$("#mouth-open").attr('class','gesture-box error');
...
const okStep = ()=>{
	...
	//Set color box
	if (STEP==1)
		scanPart.eyeOpen=true;

	if (STEP==2)
		scanPart.eyeClosed=true;

	if (STEP==3)
		scanPart.smile=true;

	if (STEP==4)
		scanPart.mouthOpen=true;

	matchBox();
	console.log(STEP,scanPart);
}

//Obtengo la url para hacer request.
const getUrl = ()=>{
	...
	return `http://127.0.0.1:3000/gestures/${gesture}`;

}

//Obtengo imagen del canvas.
const getImage = () =>{
	context.drawImage(video, 0, 0, 640, 480);
	return document.getElementById("canvas").toDataURL();
} 

//Capturo la imagen y la envio.
const captureSend = ()=>{
...
const runLoop = async ()=>{
	...
	if (COUNTER<=MAXTRIES){
		...
		console.log('FACE analysis runnnig...',STEP,'URL',getUrl());

		matchBox();
		markRunBox(STEP);

		COUNTER++;
		const response = await captureSend();

		//If is ok match success.
...
const scanLoop = ()=>{
	...
	loopId = setInterval(runLoop, 3000);
